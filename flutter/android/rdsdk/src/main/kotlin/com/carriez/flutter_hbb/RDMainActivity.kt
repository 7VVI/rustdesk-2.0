...
/**
 * RDMainActivity — the FlutterActivity used when embedding the RustDesk
 * controlled-end into a host app.
 *
 * It mirrors [MainActivity] (handle events from flutter, request
 * MediaProjection permission, drive the input/capture services), and adds:
 *   - static [getIntent] / [start] helpers carrying server config extras;
 *   - a `custom_config` invocation in [onResume] that injects the runtime
 *     configuration (id/relay/key/id/password) into the Dart side and starts
 *     the controlled service.
 *
 * There is no registration/authorization gate.
 *
...
class RDMainActivity : FlutterActivity() {
    companion object {
        ...
        internal fun start(
            ...
            start(context, getIntent(context, idServer, relayServer, key, id, password))
        }
    }

    private val channelTag = "mChannel"
    private val logTag = "mRDMainActivity"
    ...
    override fun onResume() {
        ...
        activity.runOnUiThread {
            ...
            // Inject runtime configuration and start the controlled service.
            // Keys must match the Dart-side `applyCustomConfigAndStart`.
            intent?.let {
                val map = mutableMapOf<String, String>()
                it.getStringExtra(RDMainRunner.KEY_ID_SERVER)?.let { v -> map["idServer"] = v }
                it.getStringExtra(RDMainRunner.KEY_RELAY_SERVER)?.let { v -> map["relayServer"] = v }
